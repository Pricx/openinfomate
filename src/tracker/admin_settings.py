        ...
        SettingsSectionDef(
            ...
            fields=[
                ...
                SettingFieldDef(
                    ...
                    description="Optional hard blocklist (comma-separated hosts). Empty keeps domain filtering disabled by default.",
                    example="csdn.net, zhihu.com",
                ),
                SettingFieldDef(
                    field="source_quality_min_score",
                    label="Min source score",
                    description="Optional hard filter for pushed Curated Info and alerts. Set 0 to disable and keep push decisions LLM-driven.",
                    kind="number",
                    example="0",
                ),
                SettingFieldDef(
                    ...
                    description="Optional low-tier host list. Empty keeps domain-quality push gating disabled by default.",
                    example="csdn.net",
                    advanced=True,
                ),
                SettingFieldDef(
        ...
        SettingsSectionDef(
            ...
            fields=[
                ...
                SettingFieldDef(
                    ...
                    description="Optional hard pre-filter before final reasoning selection. Keep off to let the main LLM judge the full backlog.",
                    kind="toggle",
                ),
                SettingFieldDef(
                    field="llm_curation_max_candidates",
                    label="Candidate hard cap",
                    description="Optional hard cap on candidates processed in one digest run. Set 0 to disable and drain the full backlog in prompt-safe batches.",
                    kind="number",
                    example="0",
                    advanced=True,
                ),
                SettingFieldDef(
                    field="llm_curation_max_digest",
                    label="Digest hard cap",
                    description="Optional hard cap on digest items per run. Set 0 to let the LLM decide.",
                    kind="number",
                    example="0",
                    advanced=True,
                ),
                SettingFieldDef(
                    field="llm_curation_max_alert",
                    label="Alert hard cap",
                    description="Optional hard cap on alert items per run. Set 0 to let the LLM decide.",
                    kind="number",
                    example="0",
                    advanced=True,
                ),
                SettingFieldDef(
                    field="llm_curation_input_dedupe_enabled",
                    label="Deterministic pre-LLM dedupe",
                    description="Optional URL/simhash anti-repeat filter before the LLM sees candidates. Keep off for pure LLM-first behavior.",
                    kind="toggle",
                    advanced=True,
                ),
                SettingFieldDef(
                    field="llm_curation_history_dedupe_days",
                    label="History dedupe days",
                    description="Optional deterministic anti-repeat lookback. Set 0 to disable.",
                    kind="number",
                    example="0",
                    advanced=True,
                ),
                SettingFieldDef(
                    field="llm_curation_prompt_batch_size",
                    label="Prompt batch size",
                    description="Runtime batching only; not a relevance cap. Set 0 for automatic batching.",
                    kind="number",
                    example="0",
                    advanced=True,
                ),
