    ...
    return {
        ...
        "llm.propose_topic_setup.fallback_ai_prompt": PromptTemplate(
            ...
            text_zh=(
                ...
                "- 每条 1 句“新信息/变化点” + 1 句“影响/下一步”（避免两句重复）。\n"
                "- 不确定就写“需核实”，不要编造。\n"
            ),
            text_en=(
                ...
                "Topic: {{topic_name}}\n"
                "Focus: {{brief}}\n\n"
                "Task: pick the truly high-signal items worth reading today.\n"
                ...
                "- digest: no fixed daily cap; keep every item that truly clears the high-signal bar. Do not pad, but also do not drop strong items because of a fixed quota.\n"
                "- each item must be 1 sentence 'new info/change' + 1 sentence 'impact/next' (no repetition).\n"
        ...
        "llm.propose_profile_setup.system": PromptTemplate(
            ...
            text_zh=(
                ...
                "- ai_prompt: 要求模型先读内容再判断；不做固定每日 digest 数量上限，只保留真正达到高信号门槛的内容；alert 仍应极少。\n"
            ...
            text_en=(
                ...
                "- ai_prompt: read content before deciding; do NOT impose a fixed daily digest cap; keep every item that truly clears the high-signal bar; alerts should still be rare.\n"
        ...
        "llm.propose_profile_setup.fallback_ai_prompt": PromptTemplate(
            ...
            text_zh=(
                ...
                "请基于我的 PROFILE_TEXT 所表达的真实兴趣，筛出当天真正值得读的高信号内容。\n\n"
                "规则：\n"
                ...
                "- 对与 PROFILE_TEXT 强相关的论坛/社区一线经验帖：只要带至少一个具体事实（版本号、报错、配置项、额度/价格变化、行为变化、截图结论、排障结果），即使还没有正式 benchmark 或完整复现，也可以进入 digest；只有纯求助、纯情绪、标题党或没有新增事实时才忽略。\n"
            ...
            text_en=(
                ...
                "Based on my PROFILE_TEXT, pick the truly high-signal items worth reading today.\n\n"
                "Rules:\n"
        ...
        "llm.triage_items.system": PromptTemplate(
            ...
            text_zh=(
                ...
                "- 对与 USER_PROFILE 强相关的论坛/社区一线经验帖：只要带至少一个具体事实（版本号、报错、配置项、额度/价格变化、行为变化、截图结论、排障结果），即使还没有正式 benchmark 或完整复现，也应优先保留给后续 curation；只有纯求助、纯情绪、标题党或没有新增事实时才直接丢弃。\n"
            ...
            text_en=(
                ...
                "- For community/forum frontline posts that strongly match USER_PROFILE: if they contain at least one concrete fact (version, error, config, quota/price change, behavior change, screenshot-backed conclusion, or troubleshooting outcome), keep them for later curation even if they do not yet include a formal benchmark or full reproduction.\n"
        ...
        "llm.curate_items.system": PromptTemplate(
            ...
            text_zh=(
                ...
                "- 对与 USER_PROFILE 强相关的论坛/社区一线经验帖：只要带至少一个具体事实（版本号、报错、配置项、额度/价格变化、行为变化、截图结论、排障结果），就可以进入 digest；不要把“还没有正式 benchmark / 完整复现”当成直接 ignore 的理由。只有纯求助、纯情绪宣泄、标题党或没有新增事实时才 ignore。\n"
            ...
            text_en=(
                ...
                "- For community/forum frontline posts that strongly match USER_PROFILE: allow digest when there is at least one concrete fact (version, error, config, quota/price change, behavior change, screenshot-backed conclusion, or troubleshooting outcome). Do not require a formal benchmark or full reproduction before keeping it.\n"
